...
class _ResolveBridge:
    def __init__(self):
        ...
        self.last_error = ""
        self.last_changed_spline = None
        self.last_changed_input_name = None
        self._watcher_thread = None
        self._watcher_running = False
        self._spline_snapshot = {}
        self._watcher_changed = False
        self._our_write_timestamp = 0.0
        self._cached_kfs = {}
        self._input_snapshot = {}
        # Import Resolve DLL here — before QWebEngineView loads Chromium DLLs.
        # Chromium patches the DLL environment and conflicts with fusionscript.dll.
        self._try_import()
    ...
    def _build_kf_table_from_preview(self, keyframes: list, start_frame: float, 
        ...
        if flat_value and is_physics:
            ...
            for i, kf in enumerate(keyframes):
                ...
                # Clamp: pin first keyframe to exact start_val (kills the starting long handle)
                # Last keyframe already settles to start_val via tail math
                if i == 0:
                    value = start_val
                else:
